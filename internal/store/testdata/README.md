...
| `version-5/` | `VERSION` = `5`, one user record | The 5 -> 6 migration: `machine-classes/` is created and nothing is backfilled into it. Labels are the operator's own words and nobody has written any yet, so a migration that invented some would be inventing facts. |
| `version-2/` | `VERSION` = `2`, one user record | The 2 -> 3 migration under the real `Run`: `clusters/`, `cluster-secrets/` and `machines/` are created at `0700`, the existing record survives, and the pre-migration tarball does *not* contain `cluster-secrets/`. |
| `version-1/` | `VERSION` = `1`, one user record | The whole shipped table under the real `Run`: the `schematics/` directory is created, the existing record survives, and the pre-migration tarball does *not* contain `schematics/` — which is what proves the backup was taken before the migration ran rather than after. |
